...
avg_ratings = movie_ratings.groupBy("title").agg(
    F.count("rating").alias("num_ratings"),
    F.avg("rating").alias("avg_rating")
)

# Top 10 movies with more than 50 ratings
top_movies = avg_ratings.filter("num_ratings >= 50").orderBy("avg_rating", ascending=False)
top_movies.show(10)

# Average rating per genre
movie_ratings = movie_ratings.withColumn("genre", explode(split("genres", "\\|")))
genre_ratings = movie_ratings.groupBy("genre").agg(F.avg("rating").alias("avg_genre_rating"))
genre_ratings.orderBy("avg_genre_rating", ascending=False).show()

# Save Results
output_path = "output/movie_ratings"
top_movies.write.mode("overwrite").csv(f"{output_path}/top_movies.csv", header=True)
genre_ratings.write.mode("overwrite").csv(f"{output_path}/genre_ratings.csv", header=True)
